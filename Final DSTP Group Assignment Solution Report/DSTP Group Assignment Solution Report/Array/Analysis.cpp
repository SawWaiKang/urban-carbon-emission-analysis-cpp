...
double Analysis::getTotalEmissionByAgeGroupAndMode(const DynamicArray& arr, const string& ageGroup, const string& mode) {
    double total = 0.0;
    const Record* data = arr.getData();
    int size = arr.getSize();
    for (int i = 0; i < size; i++) {
        if (data[i].getAgeGroup() == ageGroup && data[i].transportMode == mode) {
            total += data[i].calculateMonthlyEmission();
        }
    }
    return total;
}
...
double Analysis::calculateTotalEmission(const DynamicArray& arr) {
    double total = 0.0;
    const Record* data = arr.getData();
    int size = arr.getSize();
...
double Analysis::calculateTotalEmissionByCity(const DynamicArray& arr, const string& city) {
    
    
    return calculateTotalEmission(arr);
}

double Analysis::calculateEmissionByTransportMode(const DynamicArray& arr, const string& mode) {
    double total = 0.0;
    const Record* data = arr.getData();
    int size = arr.getSize();
...
void Analysis::displayEmissionByTransportMode(const DynamicArray& arr) {
    ...
    for (const string& mode : modes) {
        double emission = calculateEmissionByTransportMode(arr, mode);
        cout << setw(20) << mode << setw(25) << fixed << setprecision(2) << emission << "\n";
    }
    cout << "\n";
}

int Analysis::countTransportByAgeGroup(const DynamicArray& arr, const string& ageGroup, const string& mode) {
    int count = 0;
    const Record* data = arr.getData();
    int size = arr.getSize();
...
void Analysis::displayEmissionByAgeGroup(const DynamicArray& arr) {
    ...
    for (const string& ag : ageGroups) {
        double emission = getTotalEmissionByAgeGroup(arr, ag);
        if (emission > 0) {
...
void Analysis::displayAverageEmissionPerResident(const DynamicArray& arr) {
    string ageGroups[] = {"6-17", "18-25", "26-45", "46-60", "61-100"};
    ...
    for (const string& ag : ageGroups) {
        int count = countResidentsByAgeGroup(arr, ag);
        if (count > 0) {
            double avg = getAveragePollutionByAgeGroup(arr, ag);
...
void Analysis::displayMostPreferredTransport(const DynamicArray& arr) {
    string ageGroups[] = {"6-17", "18-25", "26-45", "46-60", "61-100"};
    ...
    for (const string& ag : ageGroups) {
        int count = countResidentsByAgeGroup(arr, ag);
        if (count > 0) {
            string preferred = getMostPreferredTransportByAgeGroup(arr, ag);
...
void Analysis::displayComparisonByCity(const DynamicArray& cityA, const DynamicArray& cityB, const DynamicArray& cityC) {
    cout << "\n" << setw(15) << "City" << setw(25) << "Total Emission (kg CO2)" << setw(20) << "Avg Emission\n";
    cout << string(60, '-') << "\n";

    DynamicArray cities[3] = {cityA, cityB, cityC};
...
int Analysis::countResidentsByAgeGroup(const DynamicArray& arr, const string& ageGroup) {
    int count = 0;
    const Record* data = arr.getData();
    int size = arr.getSize();
...
string Analysis::getMostPreferredTransportByAgeGroup(const DynamicArray& arr, const string& ageGroup) {
    string modes[] = {"Car", "Bus", "Bicycle", "Walking", "School Bus", "Carpool"};
    int maxCount = 0;
    string mostPreferred = "None";

    for (const string& mode : modes) {
        int count = countTransportByAgeGroup(arr, ageGroup, mode);
        if (count > maxCount) {
            maxCount = count;
            mostPreferred = mode;
        }
...
double Analysis::getAveragePollutionByAgeGroup(const DynamicArray& arr, const string& ageGroup) {
    int count = countResidentsByAgeGroup(arr, ageGroup);
    if (count == 0) return 0.0;

    double total = getTotalEmissionByAgeGroup(arr, ageGroup);
    return total / count;
}

double Analysis::getTotalEmissionByAgeGroup(const DynamicArray& arr, const string& ageGroup) {
    double total = 0.0;
    const Record* data = arr.getData();
    int size = arr.getSize();
...
void Analysis::analyzeDataset(const DynamicArray& arr, const string& cityName) {
    cout << "\n" << string(60, '=') << "\n";
    cout << "COMPREHENSIVE ANALYSIS - " << cityName << "\n";
    cout << string(60, '=') << "\n";

    cout << "\nTotal Records: " << arr.getSize() << "\n";
    cout << "Total Emission: " << fixed << setprecision(2) << calculateTotalEmission(arr) << " kg CO2\n";

    cout << "\n--- EMISSION BY TRANSPORT MODE ---\n";
    displayEmissionByTransportMode(arr);

    cout << "\n--- EMISSION BY AGE GROUP ---\n";
    displayEmissionByAgeGroup(arr);

    cout << "\n--- AVERAGE EMISSION PER RESIDENT (BY AGE GROUP) ---\n";
    displayAverageEmissionPerResident(arr);

    cout << "\n--- MOST PREFERRED TRANSPORT (BY AGE GROUP) ---\n";
    displayMostPreferredTransport(arr);

    cout << string(60, '=') << "\n\n";
}


void Analysis::displayAgeGroupAnalysisSummary(const DynamicArray& arr, const string& cityName) {
    string ageGroups[] = {"6-17", "18-25", "26-45", "46-60", "61-100"};
    string ageLabels[] = {
        "Children",
        "Young Adults",
    ...
    for (int i = 0; i < 5; i++) {
        ...
        int totalResidents = countResidentsByAgeGroup(arr, ag);
        ...
        for (int m = 0; m < 6; m++) {
            ...
            int count = countTransportByAgeGroup(arr, ag, mode);
            double emission = getTotalEmissionByAgeGroupAndMode(arr, ag, mode);
